...
gameReturn runGame(){
  ...
  static uint16_t carLaunchDelay_ms = 2000; //<- minimum time before next car launches (decreases)
  static uint8_t carMoveDelay_ms = 100; //<-- delay affects speed of car (decreases)
  ...
  if (gameTime_ms == 0){
    ...
    car_count = 0; carLaunchDelay_ms = 2000; carMoveDelay_ms = 100;
  ...
  if (launchCar){
      ...
      if(car_prox > ROAD_H){
        ...
        if ((car_count > 0) and (car_count % delayStep) == 0){
          carLaunchDelay_ms = max(carLaunchDelay_ms - 250,0);
          carMoveDelay_ms = max(carMoveDelay_ms - 20,0);
          //Serial.println(carMoveDelay_ms);
        }
      }
  }
...
void ISR_Pause(){
  ...
  if (millis() - lastpausepress_ms > DEBOUNCE_DELAY_MS){
    ...
    pause_pressed = 1;
    //Serial.println("I pressed the pause button");
  }
}
/*** COMMENTS ***********/
//128x64 pixels -> 30..98, 
